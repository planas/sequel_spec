...
describe "validate_length_range_matcher" do
  ...
  describe "messages" do
    context "with the old syntax" do
      ...
      describe "with options" do
        it "should contain a description" do
          @matcher = validate_length_range(1..10, :name).allowing_nil
          @matcher.description.should == "validate length of :name is included in 1..10 with option(s) :allow_nil => true"
        end

        it "should set failure messages" do
          @matcher = validate_length_range(1..10, :price).allowing_nil
          @matcher.matches? subject
          @matcher.failure_message.should == "expected Item to " + @matcher.description
          @matcher.negative_failure_message.should == "expected Item to not " + @matcher.description
        end
        ...
        it "should explicit used options if different than expected" do
          @matcher = validate_length_range(1..10, :name).allowing_blank
          @matcher.matches? subject
  ...
  describe "matchers" do
    context "with the old syntax" do
      ...
      it{ should validate_length_range(1..10, :name).allowing_nil }
      it{ should_not validate_length_range(1..10, :price) }
      it{ should_not validate_length_range(0..10, :name) }
      it{ should_not validate_length_range(1..10, :name).allowing_blank }
    end
